...
from a1_real import UnitreeA1Real, resize2d # change here??
from rsl_rl import modules #change here???
...
def get_encoder_script(logdir):
    ...
    model = getattr(modules, config_dict["runner"]["policy_class_name"])(
        num_actor_obs= unitree_real_env.num_obs, #check this
        num_critic_obs= unitree_real_env.num_privileged_obs, #check this
        num_actions= 12, #check this
        obs_segments= unitree_real_env.obs_segments, #check this
        privileged_obs_segments= unitree_real_env.privileged_obs_segments, #check this
        **config_dict["policy"], #check this
    )
...
def get_input_filter(args):
    ...
    with open(osp.join(args.logdir, "config.json"), "r") as f:
        config_dict = json.load(f, object_pairs_hook= OrderedDict) #jsin what? I thinout json was suppose to be just a policy
